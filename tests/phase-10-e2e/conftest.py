"""Phase-10 (final E2E consolidation) test fixtures.

Stack: postgres (host port 15432) + backend (in-network only) + nginx
(host port 8080) — production-shaped per `testing_strategy.md` §4 §6.
The single user-journey test drives Chromium via `pytest-playwright`
against `http://127.0.0.1:8080`; backend seeding goes through the same
nginx hop (`http://127.0.0.1:8080/api/...`) so we exercise the real
proxy configuration end-to-end.

Each phase folder is required to be self-contained
(`testing_strategy.md` §3), so the user / product / recipe /
nutrition-fact factories are duplicated here rather than imported from
another phase folder. This file is a near-copy of
`tests/phase-9-frontend-recipes-and-shopping/conftest.py` minus the
mobile-only fixtures (the journey is desktop-only;
`tests/phase-8-frontend-products/test_mobile_layout.py` covers the
small-viewport contract).
"""
...
def make_recipe(api_base_url: str):
    ...
    Mirrors the factory in `tests/phase-9-frontend-recipes-and-shopping/
    conftest.py`. `products` defaults to an empty list — `api_spec.md`
    §7.1 explicitly allows recipes with no products; their nutrition
    totals come back empty.
    """
