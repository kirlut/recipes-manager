"""Phase-7 (shopping list) test fixtures.
...
(`/shopping-list`, `/products`, `/recipes`, ...) per `api_spec.md §1.1`
and `testing_strategy.md §4.7`.

Mirrors `tests/phase-6-listing/conftest.py`. Each phase folder is
required to be self-contained (`testing_strategy.md` §3), hence the
duplication rather than an import from another phase folder.
"""
...
def make_product(api_base_url: str):
    ...
    Phase-7 tests need products as recipe ingredients (per api_spec §6.1
    every product carries ≥1 nutrition fact). The factory accepts an
    explicit `nutrition_facts` list so each test shapes the product as
    needed; the shopping list output ignores the facts entirely but the
    products endpoint requires them.
    """
...
def make_recipe(api_base_url: str):
    ...
    Phase-7 fixtures use recipes with non-empty `products` (so the
    shopping list aggregation can produce values). One aggregation test
    deliberately passes an empty list to confirm an empty recipe
    contributes nothing — that test calls `make_recipe(... products=[])`.
    """
